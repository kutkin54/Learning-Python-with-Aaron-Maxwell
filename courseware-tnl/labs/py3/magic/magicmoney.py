...
class Money:
    ...
    def __str__(self):
        return '${:0}.{:02d}'.format(self.dollars, self.cents)

    def __repr__(self):
        return 'Money({}, {})'.format(self.dollars, self.cents)

    def __add__(self, money):
        return Money(self.dollars+money.dollars+(self.cents+money.cents)//100,
                     (self.cents+money.cents) % 100)
    ...
    def __mul__(self, factor):
        return Money((self.dollars+self.cents/100)*(factor)//1,
                     (self.dollars+self.cents/100)*(factor) % 1)

    def __eq__(self, money):
        return self.dollars == money.dollars and self.cents == money.cents

    def __gt__(self, money):
        return (self-money).dollars > 0 and (self-money).cents > 0
